...
function loadConfig(configPath) {
  ...
  config.outputPath = path.join(config.outputDir, `${config.eventName.replace(/\s+/g, '_')}_Attendees_${date}.xlsx`);
  return config;
}
...
function isSwapcard(url, contentType) {
  return contentType.includes('json') && url.includes('swapcard.com');
}
